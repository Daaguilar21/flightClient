...
def main():
    ...
    try:
        print("Connected to Cassandra (insurance_booking_records).")
        while True:
            rows = session.execute("""
                SELECT customer_id, flight_id, ticket_price FROM insurance_booking_records
            ...
            for row in rows:
                print(f"[Insurance Alert] Customer {row.customer_id} booked Flight {row.flight_id} - insured flex ticket at ${row.ticket_price:.2f}.")
